...
:::tip
没有 GPU？请参见[无 GPU 快速入门](./gpu-free-quick-start.md)，在纯 CPU 集群中使用模拟推理后端，体验 Kthena 的完整请求流程。
:::
...
下面的[示例](https://raw.githubusercontent.com/volcano-sh/kthena/refs/heads/main/examples/model-serving/gpu-pd-disaggregation.yaml)使用 `ModelServing` 在 GPU 上部署 Prefill/Decode 分离的 `Qwen/Qwen3-0.6B` 模型。完整步骤及配套的 `ModelServer`、`ModelRoute` 资源请参见[使用 ModelServing 部署 Prefill-Decode 分离（vLLM、NIXL 与 LMCache）](../user-guide/prefill-decode-disaggregation/modelserving-vllm-pd-disaggregation.md)。
...
与大模型对话前，请先创建配套的 `ModelServer` 和 `ModelRoute` 资源。具体配置请参见 vLLM GPU PD 指南中的 [ModelServer 配置](../user-guide/prefill-decode-disaggregation/modelserving-vllm-pd-disaggregation.md#2-modelserver-nixl)和 [ModelRoute 配置](../user-guide/prefill-decode-disaggregation/modelserving-vllm-pd-disaggregation.md#3-modelroute-nixl)。
